...
> **Amended 2026-07-25 (v0.2.0)** with `spec.md` v0.2.0: Phase C (spin-up measurement and budget) is
> withdrawn. Phases A, B and D stand unchanged.
...
Withdrawn with FR3/FR4/AC3 by the 2026-07-25 amendment: spin-up time is not a problem in real use, so
the measurement harness and committed budget are scoped out (`spec.md` non-goals, with the revisit
...
A second fresh-context adversarial review was run on the amended bundle; its findings are reconciled
below.
