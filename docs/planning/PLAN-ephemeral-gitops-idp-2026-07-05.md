...
5. **Phase 3 external inputs (2026-07-11):** three decisions the repo could not fabricate on its own, all supplied by the user:
   - **Domain:** `idp.x45.dev`, an existing zone the user already manages on Cloudflare (zone ID `(redacted for publication)`, account ID `(redacted for publication)` - both looked up live via the Cloudflare API, not guessed).
