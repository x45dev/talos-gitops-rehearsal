...
Accepted. Phase 0 (`talosctl cluster create` + Cilium, no CAPI) is green as of 2026-07-06 - see `PRD.md` Section 6 and `PLAN-ephemeral-gitops-idp-2026-07-05.md` Phase 0 exit criteria - confirming this decision in practice, not just on paper.
...
Implementation (Phase 0, 2026-07-06) confirmed the decision: `talosctl cluster create docker` provisions a 1 control-plane + 2 worker cluster directly, needs no management cluster, and the CNI/kube-proxy-disable patch carried over unchanged from the CAPD template's `strategicPatches` content into a standalone `--config-patch` file.
The only genuinely new finding at implementation time was a Cilium version incompatibility, unrelated to CAPD - see `ADR-cilium-1-19-kubeproxyreplacement-talos-host-networking-2026-07-06.md`.
...
* `.config/capi/capd-talos-template.yaml` and `.config/kind/` are deleted as of Phase 0 going green (this change); the CNI/kube-proxy-disable patch they contained survives as `.config/talos/cni-none.patch.yaml`.

## References

* `PLAN-ephemeral-gitops-idp-2026-07-05.md` - live-probe log of the blocker chain (Docker socket, provisioning races, patch-format fix, then this finding) and the Phase 0 talosctl rewrite.
* `ZOR-ephemeral-gitops-idp-2026-07-06.md` - comparative evidence matrix (ACH) and diagnostic interview behind this decision.
* `PRD.md` Section 6 - spike status reclassification and Phase 0 green status.
